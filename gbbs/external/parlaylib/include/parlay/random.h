...
#if defined(USE_DOTMIX)
#include <cilkpub/dotmix.h>
extern cilkpub::DotMixPrime dprng;
#endif
...
struct random {
 public:
  ...
  size_t ith_rand(uint64_t i) const {
    return static_cast<size_t>(hash64(i + state));
    // __cilkrts_bump_worker_rank();
    // return static_cast<size_t>(dprng.get());
  }
...
struct random {
 public:
  ...
  random() { __cilkrts_init_dprng(); }
  random fork(uint64_t i) const { return *this; }
  random next() const { return fork(0); }
  size_t ith_rand(uint64_t i) const {
    return static_cast<size_t>(hash64(i + state));
  }
  size_t operator[](size_t i) const { return ith_rand(i); }
  size_t rand() {
    return static_cast<size_t>(__cilkrts_get_dprand());
  }
...
#if defined(USE_DOTMIX) || defined(USE_DPRAND)
  auto next_rand = [&] (size_t i) -> size_t {
    return r.rand() & mask;
  };
  auto get_pos_delayed = delayed_seq<size_t>(n, next_rand);
  auto get_pos = sequence<size_t>(get_pos_delayed.begin(), get_pos_delayed.end());
#else
